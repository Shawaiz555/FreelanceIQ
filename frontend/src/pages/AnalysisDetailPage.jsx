    ...
    <ul className="space-y-2.5">
      {flags.map((flag) => (
        <li key={flag} className="flex items-start gap-2.5 text-sm">
          {isGreen ? (
            <svg className="h-4 w-4 shrink-0 mt-0.5 text-emerald-500" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" />
            </svg>
          ) : (
            <svg className="h-4 w-4 shrink-0 mt-0.5 text-red-400" viewBox="0 0 20 20" fill="currentColor">
...
function OutcomeRecorder({ analysisId, initialOutcome }) {
  const dispatch = useDispatch();
  const [outcome, setOutcome] = useState(initialOutcome || { did_bid: false, did_win: null, actual_bid_amount: '' });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaved(true);
      dispatch(addNotification({ type: 'success', message: 'Outcome saved. This improves your AI recommendations.' }));
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
      ...
      <div className="space-y-5">
        {/* Did you bid? */}
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2.5">Did you submit a bid?</p>
          <div className="flex gap-2">
            {[true, false].map((val) => (
              <button
                key={String(val)}
                type="button"
                onClick={() => setOutcome((prev) => ({ ...prev, did_bid: val, did_win: val ? prev.did_win : null }))}
        ...
        {outcome.did_bid && (
          <>
            {/* Did you win? */}
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2.5">Did you win the contract?</p>
              <div className="flex gap-2">
                {[{ label: 'Won', val: true }, { label: 'Lost', val: false }, { label: 'Pending', val: null }].map(({ label, val }) => (
                  <button
                    key={label}
                    type="button"
            ...
            <div>
              ...
              <div className="flex items-center gap-2 max-w-xs">
                ...
                <input
                  ...
                  onChange={(e) => setOutcome((prev) => ({ ...prev, actual_bid_amount: e.target.value }))}
                  placeholder="e.g. 850"
...
const WIN_PROB_STYLE = (p) =>
  p === 'High' ? 'bg-emerald-100 text-emerald-700' :
  p === 'Medium' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700';
  ...
  if (!analysis) {
    return (
      <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
        ...
        <Link to="/analysis/history" className="text-blue-600 text-sm hover:underline mt-2 inline-block">
          ← Back to history
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {job.skills_required.map((skill) => (
            <span key={skill} className={BADGE_STYLE}>{skill}</span>
          ))}
        </div>
        {(job.budget_min || job.client_hires != null) && (
          <div className="flex flex-wrap gap-5 text-sm text-slate-500 pt-4 border-t border-slate-100">
            {job.budget_min && (
              <span>
                Budget:{' '}
                <span className="font-semibold text-slate-800">${job.budget_min}–${job.budget_max}</span>
              </span>
            )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
        ...
        {/* Details */}
        <div className="sm:col-span-3 bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-5">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Suggested bid range</p>
            <p className="text-2xl sm:text-3xl font-black text-slate-900">${result.bid_min} – ${result.bid_max}</p>
          </div>

          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Win probability</p>
            <span className={`inline-flex items-center gap-1.5 text-sm font-semibold px-3 py-1.5 rounded-full ${WIN_PROB_STYLE(result.win_probability)}`}>
          ...
          <div className="flex gap-6">
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Competition</p>
