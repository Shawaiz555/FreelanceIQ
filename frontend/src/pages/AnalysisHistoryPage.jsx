...
const ACTION_STYLE = (a) =>
  a === 'Apply'              ? 'bg-emerald-100 text-emerald-700' :
  a === 'Apply with caveats' ? 'bg-amber-100 text-amber-700'     :
                               'bg-red-100 text-red-700';

const OUTCOME_STYLE = (o) =>
  o === true  ? 'bg-emerald-100 text-emerald-700' :
  o === false ? 'bg-red-100 text-red-600'          :
                'bg-slate-100 text-slate-500';

// Returns a normalised score value regardless of analysis type
    ...
    <div className="space-y-5">
        ...
        {hasFilters && !loading && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100">
            <span className="text-xs text-slate-500">
              Showing <span className="font-semibold text-slate-700">{filtered.length}</span> of {analyses.length}
            </span>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
          ...
          <button type="button" onClick={clearFilters}
            className="mt-2 text-xs font-semibold text-blue-600 hover:underline">Clear filters</button>
        </div>
      ) : (
                ...
                {deletingId === analysis._id ? (
                  <div className="flex items-center gap-4 px-6 py-4 bg-red-50 border-l-2 border-red-400">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button onClick={(e) => handleDelete(e, analysis._id)}
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <div className="shrink-0 transition-transform duration-300 group-hover:scale-105">
                        <BidScoreGauge
                          score={score} size={56} strokeWidth={5}
                          showLabel={false} animate={false}
                          label={isMatch ? 'Match' : 'Bid'}
                        />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          {isMatch && (
                            <span className="shrink-0 text-[9px] font-black px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 uppercase tracking-wider">
                              LinkedIn
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                        {isMatch ? (
                          <span className={`text-[10px] font-black px-2.5 py-1 rounded-lg flex items-center gap-1.5 uppercase tracking-wider shadow-sm ${ACTION_STYLE(analysis.match_result?.recommended_action)}`}>
                            <span className="w-1.5 h-1.5 rounded-full bg-current opacity-70" />
                            {analysis.match_result?.recommended_action || '—'}
                          </span>
                        ) : (
                          <span className={`text-[10px] font-black px-2.5 py-1 rounded-lg flex items-center gap-1.5 uppercase tracking-wider shadow-sm ${WIN_STYLE(analysis.result?.win_probability)}`}>
                            <span className="w-1.5 h-1.5 rounded-full bg-current opacity-70" />
                            {analysis.result?.win_probability}
                          </span>
                        )}
                        {!isMatch && analysis.outcome?.did_bid && (
                          <span className={`text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-wider shadow-sm ${OUTCOME_STYLE(analysis.outcome.did_win)}`}>
                            {analysis.outcome.did_win === true ? 'Won' : analysis.outcome.did_win === false ? 'Lost' : 'Bid sent'}
                          </span>
                        )}
                        <svg className="w-5 h-5 text-slate-300 group-hover:text-blue-500 group-hover:translate-x-1 transition-all duration-300"
